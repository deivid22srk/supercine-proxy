...
//   - golang.org/x/image/font/sfnt           — parser TTF + NewFace
...
// em init() — é safe chamar sfnt.Parse em init() porque não depende de GL.
var defaultFace font.Face

func init() {
	f, err := sfnt.Parse(gomono.TTF)
	if err != nil {
		log.Fatalf("[textpanel] sfnt.Parse(gomono): %v", err)
	}
	face, err := f.NewFace(nil, &sfnt.Options{
		Size:    18, // 18pt — legível em telas de 5-6"
		DPI:     72,
		Hinting: font.HintingFull,
	})
	if err != nil {
		log.Fatalf("[textpanel] sfnt.NewFace: %v", err)
	}
	defaultFace = face
}
...
func (p *TextPanel) Draw(sz size.Event) {
	...
	// Os 4 pontos são: topLeft, topRight, bottomLeft (geom.Point).
	p.img.Draw(
		sz.Size(),
		geom.Point{X: 0, Y: 0},
		geom.Point{X: geom.Pixels(sz.WidthPx), Y: 0},
		geom.Point{X: 0, Y: geom.Pixels(sz.HeightPx)},
	)
}
